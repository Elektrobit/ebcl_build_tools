...
class TestApt:
    ...
    def setup_class(cls):
        ...
        cls.apt = Apt(
            AptDebRepo(url="http://archive.ubuntu.com/ubuntu", dist="jammy", components=["main"], arch=CpuArch.AMD64)
        )
        cls.proxy = Proxy()
    ...
    def test_trusted_repo(self):
        ...
        apt = AptDebRepo(url="http://localhost", dist="local", components=["main"], arch=CpuArch.AMD64)

        deb = apt.sources_entry(trusted=True)
        assert "trusted=yes" in deb

        apt_flat = AptFlatRepo(url="http://localhost", directory="", arch=CpuArch.AMD64)
    ...
    def test_repo_arch(self):
        ...
        apt_flat = AptFlatRepo(url="http://localhost", directory="", arch=CpuArch.ARM64)
    ...
    def test_flat_apt(self):
        apt = Apt(AptFlatRepo(url="file://" + (test_data / "flat_repo").as_posix(), directory=".", arch=CpuArch.AMD64))
        p = apt.find_package("busybox-static")
        assert p
        assert p[0].name == "busybox-static"
        assert p[0].file_url is not None
    ...
    def test_equal(self):
        ...
        b = Apt(
            AptDebRepo(url="http://archive.ubuntu.com/ubuntu", dist="jammy", arch=CpuArch.AMD64, components=["main"])
        )
        assert a != b

        b = Apt(
            AptDebRepo(
                url="http://archive.ubuntu.com/ubuntu", dist="jammy", arch=CpuArch.AMD64, components=["main", "other"]
            )
        )
        assert a != b
    ...
    def test_parse_depends(self):
        ...
        vds = parse_depends(
            "libaprutil1-dbd-sqlite3 | libaprutil1-dbd-mysql | libaprutil1-dbd-odbc | libaprutil1-dbd-pgsql",
            CpuArch.AMD64,
        )
        assert vds
        assert len(vds) == 4
        ...
        vds = parse_depends("libnghttp2-14 (>> 1.50.0) | libpcre2-8-0 (<= 10.22)", CpuArch.AMD64)
        assert vds
        assert len(vds) == 2
        ...
        vds = parse_depends("libnghttp2-14 | libpcre2-8-0 (<< 10.22)", CpuArch.AMD64)
        assert vds
        assert len(vds) == 2
        assert vds[0].name == "libnghttp2-14"
        assert vds[0].version is None
